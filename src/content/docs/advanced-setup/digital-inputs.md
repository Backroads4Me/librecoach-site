...
Many coaches have a control module that operates house systems, like the water pump, fresh water autofill, and generator start. It watches a set of on/off inputs to do it. If yours reports those inputs on the network, LibreCoach publishes each one as a sensor.

Whether you get any depends on your coach. Some modules report their inputs and some don't, and a module that only carries chassis signals (parking brake, check engine, washer fluid) is ignored, because LibreCoach doesn't cover chassis systems.
...
You'll need to name them yourself. It only takes a few minutes.

:::note[Rename them for what they do]
Find the entity under **Settings → Devices & Services → Entities** and filter by the module's device name to see them together. Open one, click the gear icon, and set a name.
:::

## Working out which is which

There are two ways. The first is easier if you have the paperwork:
...
An input reads **Unavailable** when its module stops reporting, usually because the coach is powered down, the module lost power, or the module was removed. That's deliberate. Showing the last reading would make it look current when it isn't.

They come back on their own once the module reports again. There's nothing to reset.
...
They still work everywhere else. You can put them on dashboards, use them in automations, and see their history like any other entity. They just don't clutter the device page.
